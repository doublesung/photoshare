    ...
    <div class="modal-dialog h-100 rounded-0 my-0 border-0 m-sm-auto m-0 min-w-md-80 min-w-100">
      <div class="modal-content full-height-modal-content h-100 d-flex justify-content-between align-items-center p-4">
        ...
        <div class="d-flex justify-content-md-between justify-content-center align-items-center w-100">
          ...
          <div class="d-flex">
            ...
            <div class="border-0 px-0 d-flex rounded-4 overflow-hidden">
              ...
              id="dropdownBtnSize" type="button" data-bs-toggle="dropdown" data-bs-offset="0,10" data-bs-auto-close="outside" 
              aria-expanded="false"
              >
              ...
              <ul id="dropdownMenuSize" class="dropdown-menu dropdown-menu-end py-2" aria-labelledby="dropdownBtnSize">
                <li>
                ...
                <li class="bg-transparent d-flex justify-content-center border-top mt-2">
                  <button class="btn btn-secondary text-white mt-2 rounded-4" type="button" @click="downloadImage">
                    下載選擇的收藏
                  </button>
                </li>
              </ul>
        ...
        <div class="position-relative h-80 w-100">
          <img class="w-100 h-100" v-show="!isLoading" :src="photo.src.original" alt="" @load="isLoading = false" style="object-fit: contain">
...
export default {
  ...
  methods: {
    ...
    // 計算圖片大中小的高
    calculateSize(size) {
      let originWidth = this.photo.width
      let originHeight = this.photo.height
      ...
      if(size === 'original') {
        ...
        if(originWidth === originHeight) {
          ...
          // 寬固定為1920 計算等比例的高
          let quotient = originWidth / 1920
          let height = Math.ceil(originHeight / quotient)
    ...
    // 下載圖片
    downloadImage() {
      let sizeString = this.calculateSize(this.sizeArr[this.currentSizeIndex].size)
      let width = sizeString.split(' x ')[0]
      let height = sizeString.split(' x ')[1]
      let id = this.photo.id
      let photographer = this.photo.photographer
      let url = `https://images.pexels.com/photos/${id}/pexels-photo-${id}.jpeg?auto=compress&cs=tinysrgb&fit=crop&w=${width}&h=${height}`
      let imageName = `photoshare-${photographer}-${id}.jpg`
